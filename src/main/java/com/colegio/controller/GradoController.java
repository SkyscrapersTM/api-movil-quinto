...
@RestController
@RequestMapping("/rest/colegio/grado")
public class GradoController {
	...
	public ResponseEntity<Grado> inserta(@RequestBody Grado obj){
		return ResponseEntity.ok(service.insertUpdateGrado(obj)); 
	}
	...
	@DeleteMapping("/{id}")
	public ResponseEntity<Grado> elimina(@PathVariable int id){
		Optional<Grado> obj = service.buscarPorId(id);
		if(obj.isPresent()) {
